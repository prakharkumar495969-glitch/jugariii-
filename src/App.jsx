...
Notes:
- FINAL DESIGN: Vibrant Orange/Purple Theme, Swiper Slider, All Images Integrated.
...
- NEW FEATURE: New logo and color scheme based on logo.
- NEW FEATURE: Quantity adjuster (+/- buttons) on product cards and detail modal.
*/
...
const NEW_LOGO_URL = "https://i.ibb.co/NdPhbz9z/IMG-20251006-120535.png"; // New Logo URL
// --- END: CONFIGURATION ---
...
export default function App() {
  ...
  const [tempQty, setTempQty] = useState({}); // For quantity adjustment on product cards
  ...
  useEffect(() => {
    if (selected && window.Swiper) {
        ...
        setTempQty({ ...tempQty, [selected.id]: 1 }); // Set default qty for modal if product selected
    } else {
  ...
  function handleTempQtyChange(pid, increment) {
    setTempQty(prev => {
        const current = prev[pid] || 1;
        const newQty = Math.max(1, current + increment);
  ...
  function payWithRazorpay(orderInfo) {
    ...
    const options = {
      ...
      theme: {
        color: "#f97316" // Orange color (from logo)
      }
    };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      <header className="bg-white shadow-md sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* --- UPDATED LOGO URL & Matching color scheme --- */}
            <img src={NEW_LOGO_URL} alt="JUGARIII Logo" className="w-12 h-12 rounded-full object-cover shadow-inner border-2 border-orange-200" />
            <h1 className="text-2xl font-black text-orange-600 tracking-wider">JUGARIII</h1>
          </div>
          <nav className="flex items-center gap-4">
            <button onClick={() => setShowCart((s) => !s)} className="relative p-2 rounded-full hover:bg-orange-50 transition">
              ...
              {cartCount > 0 && (
                <span className="absolute top-0 right-0 bg-orange-600 text-white rounded-full text-xs w-5 h-5 flex items-center justify-center font-bold shadow-md">{cartCount}</span>
              )}
            </button>
          </nav>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <section id="featured-products">
          <h2 className="text-3xl sm:text-4xl font-extrabold mb-10 text-center text-orange-600 font-serif">Our Featured Game Packs</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12">
            {products.map((p) => (
              <article
                key={p.id}
                className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-orange-100 cursor-pointer"
                onClick={() => setSelected(p)}
              >
                <div className="h-64 bg-gray-100 relative">
                  <img src={p.img} alt={p.title} className="w-full h-full object-cover transition duration-500 hover:scale-105" />
                  <span className="absolute top-3 right-3 bg-orange-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg transform rotate-3">ON SALE!</span>
                </div>
                <div className="p-6">
                  ...
                  <div className="mt-5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-2xl font-black text-orange-600">{currencyINR(p.price)}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      {/* --- Quantity Adjuster on Product Card --- */}
                      <div className="flex items-center border border-slate-300 rounded-full">
                            <button
                                onClick={(e) => { e.stopPropagation(); handleTempQtyChange(p.id, -1); }}
                                className="p-1 px-3 text-slate-600 hover:bg-slate-100 rounded-l-full transition font-bold text-lg"
                            >-</button>
                            <span className="px-2 text-slate-800 text-lg font-semibold">{tempQty[p.id] || 1}</span>
                            <button
                                onClick={(e) => { e.stopPropagation(); handleTempQtyChange(p.id, 1); }}
                                className="p-1 px-3 text-slate-600 hover:bg-slate-100 rounded-r-full transition font-bold text-lg"
                            >+</button>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); addToCart(p.id, tempQty[p.id] || 1); }} className="text-sm px-5 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-full font-semibold shadow-lg transition transform hover:scale-105">Add to Cart</button>
                    </div>
                  </div>
                </div>
              </article>
        ...
        <section id="why-shop" className="mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            <div className="bg-white p-8 rounded-xl shadow-xl text-center border-t-8 border-orange-600 transform hover:scale-[1.03] transition duration-300">
                <div className="text-2xl font-bold mb-2 text-orange-600">🚚 Fast & Reliable Shipping</div>
                <div className="text-slate-600 mt-2">Get your game packs delivered across India in just 3-5 business days.</div>
            </div>
            <div className="bg-white p-8 rounded-xl shadow-xl text-center border-t-8 border-orange-600 transform hover:scale-[1.03] transition duration-300">
                <div className="text-2xl font-bold mb-2 text-orange-600">🔒 100% Secure Checkout</div>
      ...
      <div className={`fixed right-0 top-0 h-full w-full sm:w-[450px] bg-white shadow-2xl transform transition-transform ${showCart ? "translate-x-0" : "translate-x-full"} z-40`}>
        <div className="p-5 border-b flex items-center justify-between bg-orange-600 text-white shadow-lg">
            ...
            return (
              <div key={pid} className="flex items-center gap-4 py-4 border-b last:border-b-0">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-orange-600 font-semibold">{currencyINR(p.price)}</div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <input className="w-16 border rounded text-center focus:border-orange-500 py-1 text-sm font-medium" type="number" value={qty} onChange={(e) => updateQty(pid, e.target.value)} min="1" />
        ...
        <div className="p-5 border-t absolute bottom-0 w-full bg-slate-50 shadow-inner">
          {!orderPlaced ? (
            <form onSubmit={placeOrder} className="space-y-4">
              ...
              <div className="p-3 bg-white rounded-xl shadow-sm space-y-2 border border-orange-200">
              ...
              <div className="space-y-1 text-sm border-b border-dashed pb-3">
                ...
                <div className="flex justify-between text-xl font-black pt-2 text-orange-700"><span>Total Payable:</span><span>{currencyINR(cartFinalTotal())}</span></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <input required className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="First Name *" name="firstName" value={checkout.firstName} onChange={handleCheckoutChange} />
                <input className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Last Name" name="lastName" value={checkout.lastName} onChange={handleCheckoutChange} />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <input required className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Mobile Number *" name="phone" value={checkout.phone} onChange={handleCheckoutChange} />
                <input className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Email (Optional)" name="email" value={checkout.email} onChange={handleCheckoutChange} />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <input required className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="State *" name="state" value={checkout.state} onChange={handleCheckoutChange} />
                <input required className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="City *" name="city" value={checkout.city} onChange={handleCheckoutChange} />
              </div>

              <textarea required className="w-full border rounded px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Full Address (House No, Street, Landmark) *" name="address" value={checkout.address} onChange={handleCheckoutChange} />

              <div className="flex gap-2 pt-3">
                <button type="submit" className="flex-1 bg-orange-600 hover:bg-orange-700 text-white px-4 py-3 rounded-xl font-bold shadow-lg transition transform hover:scale-[1.01]">
          ...
          <div className="bg-white max-w-5xl w-full max-h-[95vh] rounded-2xl overflow-y-auto shadow-2xl transform transition-all duration-300">
            <div className="p-6 sm:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    <div className="relative h-72 sm:h-96 bg-gray-100 rounded-xl overflow-hidden shadow-lg border-4 border-orange-100">
                        <div className="swiper modalSwiper w-full h-full">
                            ...
                            <div className="swiper-button-next swiper-button-next-modal text-orange-600 scale-75"></div>
                            <div className="swiper-button-prev swiper-button-prev-modal text-orange-600 scale-75"></div>
                        </div>
                    </div>
                    ...
                    {selected.gallery.map((imgUrl, index) => (
                        <img
                            ...
                            src={imgUrl}
                            className={`min-w-[70px] h-20 object-cover rounded-xl cursor-pointer border-3 transition-all shadow-md ${modalImage === imgUrl ? 'border-orange-600' : 'border-slate-300 hover:border-orange-300'}`}
                            onClick={() => {
                ...
                <div className="md:border-l md:pl-8 pt-4 md:pt-0">
                    <h3 className="text-3xl sm:text-4xl font-extrabold text-orange-600 font-serif leading-tight">{selected.title}</h3>
                    ...
                    <div className="mt-10 flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex items-center border border-slate-300 rounded-xl">
                            <button
                                onClick={() => handleTempQtyChange(selected.id, -1)}
                                className="p-2 px-4 text-slate-600 hover:bg-slate-100 rounded-l-xl transition font-bold text-xl"
                            >-</button>
                            <span className="px-4 text-slate-800 text-xl font-semibold">{tempQty[selected.id] || 1}</span>
                            <button
                                onClick={() => handleTempQtyChange(selected.id, 1)}
                                className="p-2 px-4 text-slate-600 hover:bg-slate-100 rounded-r-xl transition font-bold text-xl"
                            >+</button>
                        </div>
                        <button onClick={() => { addToCart(selected.id, tempQty[selected.id] || 1); setSelected(null); }} className="px-8 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-xl shadow-xl font-bold text-lg transition transform hover:scale-[1.02]">
                            Add to Cart
                        </button>
                    </div>
                </div>
      ...
      <footer className="mt-16 py-8 text-center text-sm text-slate-500 border-t bg-white">
          <p className="text-lg font-bold text-orange-600 mb-2">JUGARIII</p>
          <div className="flex justify-center items-center gap-4 mb-3">
            <a href={INSTAGRAM_LINK} target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:text-orange-800 transition">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c2.716 0 3.056.012 4.122.06c1.077.051 1.791.218 2.428.465.66.257 1.159.597 1.636 1.075.479.479.82 1.018 1.075 1.636.248.637.414 1.35.465 2.428.048 1.066.06 1.406.06 4.122s-.012 3.056-.06 4.122c-.051 1.077-.218 1.791-.465 2.428-.257.66-.597 1.159-1.075 1.636-.479.479-1.018.82-1.636 1.075-.637.248-1.35.414-2.428.465-1.066.048-1.406.06-4.122.06s-3.056-.012-4.122-.06c-1.077-.051-1.791-.218-2.428-.465-.66-.257-1.159-.597-1.636-1.075-.479-.479-.82-1.018-1.075-1.636-.248-.637-.414-1.35-.465-2.428-.048-1.066-.06-1.406-.06-4.122s.012-3.056.06-4.122c.051-1.077.218-1.791.465-2.428.257-.66.597-1.159 1.075-1.636.479-.479 1.018-.82 1.636-1.075.637-.248 1.35-.414 2.428-.465C8.944 2.012 9.284 2 12 2zm0 2.218c-2.616 0-2.924.01-3.951.058-1.026.049-1.666.216-2.126.398-.5.195-.889.467-1.282.86-.39.39-.665.782-.86 1.282-.182.46-.35 1.1-.398 2.126-.048 1.027-.058 1.335-.058 3.951s.01 2.924.058 3.951c.048 1.026.216 1.666.398 2.126.195.5.467.889.86 1.282.39.39.782.665 1.282.86.46.182 1.1.35 2.126.398 1.027.048 1.335.058 3.951.058s2.924-.01 3.951-.058c1.026-.048 1.666-.216 2.126-.398.195-.5.467-.889.86-1.282.39-.39.665-.782.86-1.282.182-.46.35-1.1.398-2.126.048-1.027.058-1.335.058-3.951s-.01-2.924-.058-3.951c-.048-1.026-.216-1.666-.398-2.126-.195-.5-.467-.889-.86-1.282-.39-.39-.782-.665-1.282-.86-.46-.182-1.1-.35-2.126-.398-1.027-.048-1.335-.058-3.951-.058zm0 3.238c-3.176 0-5.75 2.574-5.75 5.75s2.574 5.75 5.75 5.75 5.75-2.574 5.75-5.75-2.574-5.75-5.75-5.75zm0 9.262c-1.933 0-3.5-1.567-3.5-3.5s1.567-3.5 3.5-3.5 3.5 1.567 3.5 3.5-1.567 3.5-3.5 3.5zm7.398-9.98c0 .546.444.99.99.99h.012c.546 0 .99-.444.99-.99s-.444-.99-.99-.99h-.012c-.546 0-.99.444-.99.99z"/></svg>
            </a>
          </div>
          <p>&copy; {new Date().getFullYear()} JUGARIII. All rights reserved.</p>
          <p className="mt-1 text-xs">Jugariii Store | Developed by a friend.</p>
      </footer>
    </div>
  );
                    }
